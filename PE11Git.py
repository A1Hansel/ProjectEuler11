...
for i in range(0,Q): #sweeps along chart
    for n in range(0,Q-3): 
#Checking horriz. products
        p = np.product(A[i,n:n+4])  #A[down,right]
        if p > max:
           max = p
#Checking vert. products
        p = np.product(A[n:n+4,i])
        if p > max:
            max = p
